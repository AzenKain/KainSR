...
import { onGetAvatarDataCsReq, onGetAvatarDataCsReqNew, onGetBagCsReq, onGetBagCsReqNew, onGetCurLineupDataCsReq, onGetCurSceneInfoCsReq, onGetCurSceneInfoCsReqFix } from '.';
...
export async function onSendMsgCsReq(
    ...
    else if (body.messageText.startsWith("/reload")) {
        const data = body.messageText.split(' ')
        const idChar = data[1]
        if (idChar.toLocaleLowerCase() == 'notfix') {

            await onGetCurSceneInfoCsReq({}, player, dataModule)
        }
        else if ((idChar.toLocaleLowerCase() == 'fix') ){
            await onGetCurSceneInfoCsReqFix({}, player, dataModule)
